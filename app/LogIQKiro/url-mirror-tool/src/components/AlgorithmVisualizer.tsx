...
import { Play, Pause, RotateCcw, SkipBack, SkipForward, BarChart3, Circle, TrendingUp, Keyboard, Heart, Star } from "lucide-react";
          ...
          <CardContent>
            <Select value={algorithm} onValueChange={setAlgorithm}>
              ...
              <SelectContent className="bg-slate-800 border-slate-700 max-h-80">
                ...
                {favorites.length > 0 && (
                  <>
                    ...
                    {favorites.map((favAlgo) => (
                      <SelectItem key={`fav-${favAlgo}`} value={favAlgo} className="text-white pl-4 bg-red-900/10">
                ...
                <SelectItem value="bubble" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Bubble Sort</span>
                    {isFavorite('bubble') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                <SelectItem value="selection" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Selection Sort</span>
                    {isFavorite('selection') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                <SelectItem value="insertion" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Insertion Sort</span>
                    {isFavorite('insertion') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                <SelectItem value="merge" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Merge Sort</span>
                    {isFavorite('merge') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                <SelectItem value="quick" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Quick Sort</span>
                    {isFavorite('quick') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                <SelectItem value="heap" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Heap Sort</span>
                    {isFavorite('heap') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                <SelectItem value="shell" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Shell Sort</span>
                    {isFavorite('shell') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                <SelectItem value="counting" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Counting Sort</span>
                    {isFavorite('counting') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                <SelectItem value="radix" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Radix Sort</span>
                    {isFavorite('radix') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                <SelectItem value="bucket" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Bucket Sort</span>
                    {isFavorite('bucket') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                ...
                <SelectItem value="linear" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Linear Search</span>
                    {isFavorite('linear') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                <SelectItem value="binary" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Binary Search</span>
                    {isFavorite('binary') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                ...
                <SelectItem value="bfs" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Breadth-First Search</span>
                    {isFavorite('bfs') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
                <SelectItem value="dfs" className="text-white pl-4">
                  <div className="flex items-center justify-between w-full">
                    <span>Depth-First Search</span>
                    {isFavorite('dfs') && <Heart className="w-3 h-3 fill-current text-red-400" />}
                  </div>
                </SelectItem>
              </SelectContent>
            </Select>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <Badge variant="outline" className="text-blue-400 border-blue-400">{stats.comparisons}</Badge>
              </div>
              <div className="flex justify-between">
                <span className="text-purple-400">SWAPS</span>
                <Badge variant="outline" className="text-purple-400 border-purple-400">{stats.swaps}</Badge>
              </div>
            </div>
